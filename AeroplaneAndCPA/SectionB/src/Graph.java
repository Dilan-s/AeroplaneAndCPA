...
public class Graph implements GraphInterface {
  ...
  private void setIncomingDegreeRecursively(GraphNode node) {
    ...
    for (GraphEdge g : node.getOutgoingEdges()) {
      setIncomingDegreeRecursively(g.getEndPoint());
    }
  }
  ...
  private QueueInterface<GraphNode> topologicalSort() {
    ...
    temp.enqueue(start);
    while (!temp.isEmpty()) {
      GraphNode g = temp.dequeue();
      result.enqueue(g);
      for (GraphEdge gs : g.getOutgoingEdges()) {
        gs.getEndPoint().setDegree(gs.getEndPoint().getDegree() - 1);
        if (gs.getEndPoint().getDegree() == 0) {
          temp.enqueue(gs.getEndPoint());
        }
      }
    }
    return result;
  ...
  private void computeEarliestCompletionTime() {
    ...
    while (!sortedNodes.isEmpty()) {
      for (GraphEdge gs : start.getOutgoingEdges()) {
        int currentEc = gs.getEndPoint().getEarliestCompletionTime();
        gs.getEndPoint().setEarliestCompletionTime(Math.max(currentEc,
            gs.getTaskDuration() + gs.getStartPoint().getEarliestCompletionTime()));
      }
